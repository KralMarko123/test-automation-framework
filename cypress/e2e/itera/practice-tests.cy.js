...
describe("Practice test suite", () => {
	...
	it("Checkbox & Radio button practice", () => {
		...
		testAutomationPage.visit();
		testAutomationPage.getRadioButtons().check(["option1", "option2", "option3"], { force: true });
		testAutomationPage.getFemaleRadio().check().should("be.checked");
		testAutomationPage.getCheckboxWithId("tuesday").check().should("be.checked");
	...
	it("Dropdown practice", () => {
		...
		testAutomationPage.visit();
		while (count > 0) {
			testAutomationPage.getDropdown().select(count).should("have.value", count);
			count--;
		}
